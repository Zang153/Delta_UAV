...
mass = 1.27           # 飞行器质量 单位kg
# Ct = 10           # 电机推力系数 (N/krpm^2)
# Cd = 7.9379e-2          # 电机反扭系数 (Nm/krpm^2)

# arm_length = 0.18  # 电机力臂长度 单位m
# max_thrust = 64.856     # 单个电机最大推力 单位N (电机最大转速22krpm)
# max_torque = 38.42  # 单个电机最大扭矩 单位Nm (电机最大转速22krpm)
...
def load_callback(m=None, d=None):
    ...
    m = mujoco.MjModel.from_xml_path('/home/zyx/Documents/Learn/Test-UAV/Delta.xml')
    d = mujoco.MjData(m)
    if m is not None:
...
# 初始化电机动力分配器
mixer = Mixer()
torque_scale = 0.0001 # 控制器控制量到实际扭矩(Nm)的缩放系数(因为是无模型控制所以需要此系数)
...
def control_callback(m, d):
    ...
    goal_pos, goal_heading = np.array([0.0, 0.0, 0.7]), np.array([0.0, 1.0, 0.0])  # 目标位置
    ...
    # 构建当前状态
    curr_state = State(_pos[:3], _vel[:3], quat, omega)
    ...
    d.ctrl[0] = calc_motor_input(motor_speed[0])
    d.ctrl[1] = calc_motor_input(motor_speed[1])
    d.ctrl[2] = calc_motor_input(motor_speed[2])
    d.ctrl[3] = calc_motor_input(motor_speed[3])

    # d.ctrl[0] = motor_speed[0]
    # d.ctrl[1] = motor_speed[1]
    # d.ctrl[2] = motor_speed[2]
    # d.ctrl[3] = motor_speed[3]
